...
0.3.0. It is a claim ledger, not a coverage percentage. The 187 normative rules
define review obligations; no test count implies one test per rule or universal
proof.

## Executable baseline

- 38 ordinary unit tests exercise the doctrine example crates.
- Eight `trybuild` UI cases preserve selected compiler rejections.
...
| [RUST-DOC-0010](doctrines/0010-staged-protocols/README.md)            |    22 | `staged-protocol` exercises two entry paths, both branches, both recovery edges, undetermined availability, and stale consent; a topology assertion pins every documented edge; three compiler-rejection cases cover stage skipping, consumed-stage reuse, and evidence forgery | unit plus compile-fail plus topology assertion | no database, competing writer, restoration path, async cancellation, or durable-advancement test; R014, R015, and R016 have no executable evidence here |
...
selected program remains rejected at the pinned diagnostic boundary. A topology
assertion proves the protocol edges it names still typecheck; it does not prove
the graph is the right graph for a domain, which remains a review judgment. Miri
